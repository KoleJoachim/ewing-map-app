...
def get_chemical_columns():
    ...
    numerical = ["Rate_per_1M", "Count", "Population"]  # Explicit numerical columns
    categorical = [
        col for col in df.columns 
        if col.endswith("_label") and df[col].dtype == "int64"
    ]
...
def index():
    try:
        ...
        if selected_chemical in numerical_cols:
            ...
            # Treat binary (0/1) as categorical for discrete colors
            fig = px.choropleth(
                final_df,
                geojson=GEOJSON,
                locations="FIPS",
                color=selected_chemical.astype("category"),
                color_discrete_sequence=["green", "red"],  # Low=green, High=red
        ...
        <h1>Error</h1>
        <p>{str(e)}</p>
        <p>Ensure you're selecting valid columns. Numerical columns show intensity, 
        categorical columns (0=Low, 1=High) show risk levels.</p>
        """, 500
